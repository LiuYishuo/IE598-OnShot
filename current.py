...
mnist = input_data.read_data_sets("/u/training/tra250/scratch/project/rnn/MNIST_data/", one_hot=True)
...
accuracy_next_100 = tf.Variable(tf.constant(0.0),trainable=False)
...
with tf.variable_scope("RNN"):
    for time_step in range(num_steps):
        ...
        write_key_temp = linear_layer(output,num_hidden,num_read_heads*key_size)
        write_key = tf.reshape(write_key_temp,shape=[-1,num_read_heads,key_size])

        alpha = linear_layer(output,num_hidden,num_read_heads)
...
train_step = tf.train.RMSPropOptimizer(5e-4,decay=0.95,momentum=0.9).minimize(error)
...
saver.restore(sess,'/u/training/tra250/scratch/project/rnn/model_60000.ckpt')

num_episodes = 100000
for episode in range(60000,num_episodes):
    if episode%500 == 0:
